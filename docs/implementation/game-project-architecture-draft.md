...
- The initial 3d runtime slice uses the registered `static-body-3d` and
  ...
  `box-shape-3d`, `sphere-shape-3d`, `capsule-shape-3d`, or static
  `triangle-mesh-shape-3d` resource. These type descriptors are the same
  metadata used by runtime validation and the future editor Inspector.
...
- Collision-node scaling is not part of this first slice because the existing
  ...
  shape resources.
- Static triangle meshes contain immutable indexed positions and triangles.
  The physics module owns transformed broad-phase bounds, double-sided
