...
public final class BoxHelper extends GeneratedLineSegmentsHelper {
    ...
    public BoxHelper(Object3D target, Color color) {
        super(createResources(target, color));
        this.target = target;
        setRenderOrder(1);
    }
    ...
    /** Validates constructor state and creates both owned resources atomically. */
    private static OwnedResources createResources(Object3D target, Color color) {
        Object3D validTarget = Objects.requireNonNull(target, "target");
        Color validColor = Objects.requireNonNull(color, "color");
        BoundingBox bounds = requireInitialBounds(validTarget);
        return new OwnedResources(createGeometry(bounds), new LineBasicMaterial(validColor));
    }
