...
public final class EditorProjectBuildSession implements AutoCloseable {
     ...
     */
    public EditorProjectBuildSession(EditorExtensionHost extensions, WorkspaceBuildPreferences preferences) {
        this(extensions, preferences, EditorProjectBuildSession::selectAdapter, EditorProjectBuildSession::dispatch);
    }
    ...
    private void publish(ProjectBuildCoordinator source, ProjectBuildSnapshot snapshot) {
        uiDispatcher.accept(() -> {
            if (coordinator == source) {
                commands.update(availability(snapshot));
            }
        });
    }
    ...
    private static ProjectBuildCommandAvailability availability(ProjectBuildSnapshot snapshot) {
        ProjectBuildPhase phase = snapshot.phase();
        boolean building = phase == ProjectBuildPhase.QUEUED || phase == ProjectBuildPhase.BUILDING;
        return new ProjectBuildCommandAvailability(true, building, false);
    }
