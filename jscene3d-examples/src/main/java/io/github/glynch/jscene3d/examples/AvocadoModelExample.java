...
public final class AvocadoModelExample {
    ...
    static HostedExample create(ExampleContext context) {
        LoadedGltf loaded = GltfLoader.load(resourcePath(MODEL_RESOURCE));
        EnvironmentMap environmentMap = EnvironmentMapLoader.load(resourcePath(ENVIRONMENT_RESOURCE));
    ...
    /** Resolves one required bundled asset while running from the examples build. */
    private static Path resourcePath(String resourceName) {
        URL resource = Objects.requireNonNull(AvocadoModelExample.class.getResource(resourceName), resourceName);
        try {
            return Path.of(resource.toURI());
        } catch (URISyntaxException exception) {
            throw new IllegalStateException("Invalid bundled asset URI: " + resourceName, exception);
        }
    }
