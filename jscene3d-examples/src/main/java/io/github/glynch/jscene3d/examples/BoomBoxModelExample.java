...
/** Displays the CC0 Boom Box glTF asset with HDR image-based lighting. */
public final class BoomBoxModelExample {
    private static final Color BACKGROUND = Color.srgb(0x10141b);
    private static final String MODEL_RESOURCE = "/io/github/glynch/jscene3d/examples/boom-box/BoomBox.glb";
    ...
    private BoomBoxModelExample() {
        throw new AssertionError("BoomBoxModelExample cannot be instantiated");
    }
    ...
    public static void main(String[] arguments) {
        ExampleLauncher.launch("JScene3D - Boom Box glTF Model", BoomBoxModelExample::create);
    }
    ...
    static HostedExample create(ExampleContext context) {
        LoadedGltf loaded =
                GltfLoader.load(path(BoomBoxModelExample.class.getResource(MODEL_RESOURCE), MODEL_RESOURCE));
        EnvironmentMap environmentMap = EnvironmentMapLoader.load(
                path(BoomBoxModelExample.class.getResource(ENVIRONMENT_RESOURCE), ENVIRONMENT_RESOURCE));
    ...
    private static ControlPanel createPanel(ExampleContext context, ExampleState settings) {
        ControlPanel panel = new ControlPanel(context.window(), "Boom Box");
    ...
    /** Mutable explicit GUI bindings for the Boom Box presentation. */
