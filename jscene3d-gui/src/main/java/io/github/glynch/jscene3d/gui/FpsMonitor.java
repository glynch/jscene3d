...
public final class FpsMonitor implements Overlay {
    ...
    /**
     * Positions the monitor's upper-left corner in logical window coordinates.
     *
    ...
    public void setPosition(float x, float y) {
        this.x = Preconditions.requireFinite(x, "x");
        this.y = Preconditions.requireFinite(y, "y");
    }
    ...
    void paint(GuiCanvas canvas, int width, int height) {
        ...
        canvas.roundedRectangle(x + 3.0f, y + 5.0f, WIDTH, HEIGHT, 8.0f, theme.shadow(), 0.38f);
        canvas.roundedRectangle(x, y, WIDTH, HEIGHT, 8.0f, theme.border(), 1.0f);
        canvas.roundedRectangle(x + 1.0f, y + 1.0f, WIDTH - 2.0f, HEIGHT - 2.0f, 7.0f, theme.panel(), 0.96f);

        String fps = framesPerSecond + " FPS";
        FONT.text(canvas, x + 10.0f, y + 8.0f, fps, 15.0f, theme.text());
        String range = minimumFramesPerSecond + " - " + maximumFramesPerSecond;
        float rangeX = x + WIDTH - 10.0f - FONT.width(range, 11.0f);
        FONT.text(canvas, rangeX, y + 11.0f, range, 11.0f, theme.mutedText());

        float graphX = x + 10.0f;
        float graphY = y + 38.0f;
        canvas.roundedRectangle(graphX, graphY, GRAPH_WIDTH, GRAPH_HEIGHT, 4.0f, theme.title(), 0.92f);
        canvas.line(
                graphX + 4.0f,
                graphY + GRAPH_HEIGHT * 0.5f,
                graphX + GRAPH_WIDTH - 4.0f,
                graphY + GRAPH_HEIGHT * 0.5f,
                1.0f,
                theme.border(),
                0.55f);
        paintGraph(canvas, graphX, graphY);
    }
    ...
    private void paintGraph(GuiCanvas canvas, float graphX, float graphY) {
        if (sampleCount < 2) {
            return;
        }
        ...
        for (int offset = 0; offset < sampleCount; offset++) {
            ...
            float sampleX = graphX + 4.0f + fraction * (GRAPH_WIDTH - 8.0f);
            float normalized = Math.clamp(samples[sampleIndex] / maximum, 0.0f, 1.0f);
            float sampleY = graphY + GRAPH_HEIGHT - 4.0f - normalized * (GRAPH_HEIGHT - 8.0f);
            if (offset > 0) {
                canvas.line(previousX, previousY, sampleX, sampleY, 1.5f, theme.accent(), 1.0f);
            }
            previousX = sampleX;
            previousY = sampleY;
        }
