...
final class TextureResource implements AutoCloseable {
    ...
    void synchronize(Texture texture, RenderStatistics statistics) {
        glBindTexture(GL_TEXTURE_2D, textureName);
        long imageVersion = texture.imageVersion();
        if (uploadedImageVersion != imageVersion) {
            uploadImage(texture, statistics);
    ...
    private void uploadImage(Texture texture, RenderStatistics statistics) {
        ...
        glTexImage2D(
                ...
                GL_UNSIGNED_BYTE,
                pixels);
        statistics.recordTextureUpload(byteCount);
    }
